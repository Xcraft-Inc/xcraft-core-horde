...
class Slave {
  ...
  *connect(busConfig, next) {
    ...
    if (busConfig.noForwarding) {
      console.log('NO FORWARDING FOR ', process.env.WESTEROS_APP);
      return;
    }
    this._busClient.events.catchAll((topic, msg) => {
      ...
      console.log('SEND BROADCAST ', process.env.WESTEROS_APP, ' ', topic);
      if (msg._xcraftRawMessage) {
        this._horde.broadcast(this.id, topic, msg._xcraftRawMessage);
      } else {

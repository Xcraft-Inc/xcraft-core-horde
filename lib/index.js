...
class Slave extends EventEmitter {
  ...
  *connect(busConfig, next) {
  ...
  busConfig(pid) {
    ...
    const westerosApp = xHost.variantId
      ? `${this.horde}@${xHost.variantId}`
      : this.horde;
    const westerosAppPath = xHost.variantId
      ? `${this.horde}-${xHost.variantId}`
      : this.horde;
    let prevWesterosApp;

    try {
      prevWesterosApp = process.env.WESTEROS_APP;
      process.env.WESTEROS_APP = westerosApp;

      const root = path.join(xHost.appData, this._company, westerosAppPath);

      require('xcraft-server/lib/init-env.js').initEtc(
        path.resolve(xHost.appConfigPath, '..', westerosAppPath),
        xHost.projectPath,
        this.horde
      );
      ...
      process.env.WESTEROS_APP = prevWesterosApp;
    }
  }

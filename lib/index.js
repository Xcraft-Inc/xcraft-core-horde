...
class Horde {
  ...
  setMaxLagDeltaTime(delta = 20000) {
    this.#maxDelta = delta;
  }
  ...
  hasSyncing(hordeId) {
    return !this._config.topology?.[hordeId]?.noSync;
  }
  ...
  broadcast(hordeId, topic, msg) {
    ...
    for (const id of this._slaves.keys()) {
      ...
      if (slave.isPassive && !slave.isConnected) {
        continue;
      }
  ...
  *autoload(resp) {
  ...
  /**
   * Wait for the connections (with timeout) if the server seems alive.
   * @param {number} timeout
   * @returns {Promise<void>}
   */
  async waitAutoload(timeout = 5000) {
    const {setTimeout: setTimeoutAsync} = require('node:timers/promises');

    for (let t = 0; t < timeout; t += 200) {
      const result = await Promise.race([
        Promise.all(this.#connects.values()),
        setTimeoutAsync(200, 'ETIMEOUT'),
      ]);
      if (result !== 'ETIMEOUT') {
        return;
      }

      /* If timeout, maybe there is an error and in this case we should
       * not wait anymore. It prevent a useless timeout when the server
       * is not accessible (offline).
       */
      const slaves = this.getSlaves().map((slave) => this.getSlave(slave));
      for (const slave of slaves) {
        if (slave.lastErrorReason) {
          return;
        }
      }
    }
  ...
  *add(slave, horde, busConfig, next) {
    ...
    if (busConfig) {
      ...
      if (busConfig.passive) {
        this.#connects.set(slave.id, promise);
      } else {
        yield promise;
      }
    } else {

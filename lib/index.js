...
class Slave extends EventEmitter {
  constructor(horde, resp, hordeId, tribe = 0) {
    super();
  ...
  start(next) {
    ...
    this._daemon = new Daemon(
      ...
      host,
      {
        detached: false,
        inspectPort: _port++,
        argv: [`--app=${appId}`, `--tribe=${this._tribe}`],
      },
      true,
      this._resp
    );
...
class Horde {
  ...
  _commands(full) {
    return Object.assign(
      ...
      ...Array.from(this._slaves.values())
        ...
        .map((slave) => {
          return {[slave.horde]: slave.commands};
        })
    );
  }
  ...
  *autoload(resp, topology, next) {
    ...
    for (let horde of this._config.hordes) {
      ...
      yield this.add(slave, horde, topology);
  ...
  *unload(resp, next) {
  ...
  getSlaves() {
    return Array.from(this._slaves.values()).map((slave) => slave.horde);
  }

...
function isLocalhost(address) {
  ...
  return (
    ...
    address.match(/^127(?:\.(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)){3}$/)
  );
}
...
class Horde {
  ...
  *add(resp, horde, topology, next) {
    ...
    if (this._topology[def]) {
      const hosts = [
        this._topology[def].commanderHost,
        this._topology[def].notifierHost,
        this._topology[def].host,
      ];

      /* Disable if at least one host is loopback */
      if (hosts.every((host) => !isLocalhost(host))) {
        slave.on('heartbeat', () => {
          if (!slave.isConnected) {
            return;
          }

          const newTimestamp = process.hrtime.bigint();
          if (timestamp) {
            if (jitterTimeout) {
              clearInterval(jitterTimeout);
              attempt = 0;
            ...
            let jitter =
              parseInt((newTimestamp - timestamp) / BigInt(1e6)) - 500; // ms
            if (jitter < 0) {
              jitter = 0;
            }
            const payload = {horde, jitter, noJitter: false, overlay: false};
            xBusClient.getGlobal().events.send('greathall::<jitter>', payload);

            jitterTimeout = setInterval(() => {
              attempt = attempt ? attempt + 1 : 1;
              const payload = {horde, noJitter: true, overlay: attempt >= 8};
              xBusClient
                .getGlobal()
                .events.send('greathall::<jitter>', payload);

              if (attempt === 16 && process.env.NODE_ENV !== 'development') {
